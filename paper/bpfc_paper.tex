...
Empirically (BERT proxy, $N=120$, $K=8$), $\sigspan$ achieves
AUROC $= 0.809 \pm 0.015$ for predicting factual errors.
A simulation study ($N=300$, 10 seeds) confirms AUROC $= 0.719 \pm 0.021$ under the BPFC
generative model.
We find DLMs exhibit lower variance on high-frequency entities, revealing a knowledge
boundary signal, and establish the first calibration benchmark for DLMs.
\end{abstract}
