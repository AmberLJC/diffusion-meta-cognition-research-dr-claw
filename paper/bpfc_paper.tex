...
Three-way cross-architecture validation confirms the signal generalizes across the MLM family:
DistilBERT-base (66M, AUROC $= 0.835$), BERT-base (110M, AUROC $= 0.791$),
and RoBERTa-large (355M, AUROC $= 0.642$).
Signal strength is \emph{inversely correlated} with model scale,
consistent with a ``compression amplifies uncertainty'' effect from distillation dynamics.
