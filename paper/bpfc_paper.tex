...
Five-way cross-architecture validation confirms signal generalization across the MLM family:
ALBERT-large-v2 (18M, AUROC $= \mathbf{0.946}$), DistilBERT-base (66M, AUROC $= 0.835$),
BERT-base (110M, AUROC $= 0.791$), ALBERT-base-v2 (12M, AUROC $= 0.679$),
and RoBERTa-large (355M, AUROC $= 0.642$).
ALBERT's cross-layer parameter sharing yields the strongest signal,
...
A five-way architecture sweep confirms cross-model generalization (Table~\ref{tab:arch_compare}):
ALBERT-large-v2 (18M, AUROC $= 0.946$) achieves the strongest signal,
with ALBERT's cross-layer parameter sharing identified as a key driver ---
the \emph{posterior-sharing hypothesis}.
...
ALBERT-large-v2  &  18 & \textbf{0.946} & [0.881, 0.994] & \textbf{2.205} & 0.22 & Cross-layer sharing \\
DistilBERT-base  &  66 & 0.835 & [0.704, 0.939] & 1.221 & 0.40 & Distilled $\dagger$ \\
...
\midrule
\multicolumn{7}{l}{Spearman $\rho(\text{params},\, \text{AUROC}) = -0.40$ (weak; not monotone)} \\
\bottomrule
\end{tabular}
\end{table}
